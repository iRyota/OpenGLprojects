...
void display(void)
{
  ...
  // --- 課題 ---
  // マウス入力で視点が移動できるようにする
  glTranslatef(-1 * yaw, pitch, -1 * distance );
  ...
  glPushMatrix();
  {
    ...
    glPushMatrix();
    {
        glRotatef(rev_vel_mercury * day * 20, cos(M_PI / 6), sin(M_PI / 6), 0.0);
        glTranslatef(0.0, 0.0, orbit_radius_mercury);

        glRotatef(rot_vel_mercury * day, cos(M_PI / 5), sin(M_PI / 5), 0.0);
    ...
    glPushMatrix();
    {
        glRotatef(rev_vel_venus * day * 300, 0.0, 1.0, 0.0);
        glTranslatef(orbit_radius_venus, 0.0, 0.0);

        glRotatef(rot_vel_venus * day, 0.0, 1.0, 0.0);
    ...
    glPushMatrix();
    {
      ...
      glPushMatrix();
      {
        glRotatef(rev_vel_moon * day * 20, cos(M_PI / 5), sin(M_PI / 5), 0.0);
        glTranslatef(orbit_radius_moon, 0.0, 0.0);

        glRotatef(rot_vel_moon * day, cos(M_PI / 5), sin(M_PI / 5), 0.0);
      ...
      glPushMatrix();
      {
        glRotatef(rev_vel_moon * day, cos(M_PI / 3), sin(M_PI / 3), 0.0);
        glTranslatef(-1 * orbit_radius_moon, 0.0, 0.0);

        glRotatef(rot_vel_moon * day, cos(M_PI / 3), sin(M_PI / 3),0.0);
    ...
    glPushMatrix();
    {
      ...
      glRotatef(rot_vel_mars * day, 0.0, 1.0, 0.0);
